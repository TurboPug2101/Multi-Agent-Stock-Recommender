    ...
    def fetch_news(self, symbol: str, company_name: str, days: int = 14) -> List[NewsArticle]:
        ...
        try:
            ...
            if 'articles' in data and 'results' in data['articles']:
                ...
                # If we need more articles and there are more pages, fetch additional pages
                # total_results = data.get('articles', {}).get('totalResults', 0)
                # total_pages = data.get('articles', {}).get('pages', 1)
                
                # if total_pages > 1 and len(articles) < min(100, total_results):
                #     # Fetch additional pages (limit to 3 pages = 300 articles max)
                ...
                #                 if 'articles' in page_data and 'results' in page_data['articles']:
                #                     for item in page_data['articles']['results']:
                #                         try:
                #                             published_date = None
                #                             if 'dateTime' in item:
                #                                 try:
                #                                     published_date = datetime.fromisoformat(item['dateTime'].replace('Z', '+00:00')).isoformat()
                #                                 except:
                #                                     pass
                                            
                #                             source_name = None
                #                             if 'source' in item and isinstance(item['source'], dict):
                #                                 source_name = item['source'].get('title') or item['source'].get('uri', '')
